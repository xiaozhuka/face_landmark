...
model_config.summary_path = r'E:\python_vanilla\log_dir\test_2018_4_26_1'
...
model_config.restore = True
model_config.bn = False
model_config.loss_amp = 20.0
...
with tf.device('/gpu:0'):
    ...
    ################## get dataset ############################
    ###########################################################
    _, test_ds, info = get_dataset_from_h5(model_config)
    finetune_ds = get_finetune_dataset_from_file(model_config)
    train_iterator = finetune_ds.make_initializable_iterator()
    ...
    boundaries = [x for x in np.array([int(42000),
                                         int(61000)],
                                        dtype=np.int32)]
    ...
    for epoch_idx in range(model_config.max_epoch):
        ...
        while True:
            ...
            try:
                ...
                #                                -1)
                #                 imwrite(os.path.join(model_config.summary_path, "finetune_test_%d_steps_%d_%d.jpg"%(total_step, tmp_test_count, tmp_idx)), img_ori.astype(np.uint8))
                #
                #     except tf.errors.OutOfRangeError:
